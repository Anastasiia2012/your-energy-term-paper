(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))a(t);new MutationObserver(t=>{for(const n of t)if(n.type==="childList")for(const r of n.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function e(t){const n={};return t.integrity&&(n.integrity=t.integrity),t.referrerPolicy&&(n.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?n.credentials="include":t.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function a(t){if(t.ep)return;t.ep=!0;const n=e(t);fetch(t.href,n)}})();async function c(s,o,e=!0){const a=new URLSearchParams({limit:12,page:o||1});return fetch(e?s+`&${a}`:s).then(t=>{if(!t.ok)throw new Error(t.status);return t.json()})}function y(){document.querySelectorAll("#pagination button").forEach(o=>{o.addEventListener("click",e=>{let a=e.target.getAttribute("data-filter");const t=parseInt(e.target.textContent);p[a.toLowerCase()].execute(t)})})}function f(s,o){const e=document.getElementById("pagination");let a="";s=s>5?5:s;for(let t=1;t<=s;t++)a+=`
    <button
      ...
      `;e.innerHTML=a,y()}function k(){const o=document.getElementById("rating").querySelectorAll("button");let e=0,a=0;function t(n){o.forEach((r,i)=>{const l=r.querySelector("svg"),d=i<n;l.classList.toggle("fill-amber-400",d),l.classList.toggle("text-amber-400",d),l.classList.toggle("text-muted-foreground/50",!d);const b=document.getElementById("rating-amount");b.innerText=n})}o.forEach((n,r)=>{const i=r+1;n.addEventListener("click",()=>{e=i,t(e)}),n.addEventListener("mouseenter",()=>{a=i,t(a)}),n.addEventListener("mouseleave",()=>{a=0,t(e)})})}function L(s=string){const o=`
<div data-state="open"
...
`;document.getElementById("popup").innerHTML=o,k();const e=document.getElementById("close-popup");e.addEventListener("click",()=>{document.getElementById("popup").innerHTML=""}),document.getElementById("review-form").addEventListener("submit",t=>{t.preventDefault();const n=document.getElementById("rating-amount").innerText;fetch(`https://your-energy.b.goit.study/api/exercises/${s}/rating`,{method:"PATCH",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({rate:parseInt(n),email:t.target.email.value,review:t.target.review.value})}).then(r=>r.json()).then(r=>{}).catch(r=>{console.error("Error:",r)}),e.click()})}function v(s){const o=document.getElementById("renderer"),e=s.results.map(({bodyPart:a,burnedCalories:t,name:n,rating:r,target:i,time:l,_id:d})=>`
    ...
    ${e}
  </div>`,f(s.totalPages,"Exercise"),o.querySelectorAll(".start-dialog").forEach(a=>{a.addEventListener("click",()=>{E(a.getAttribute("data-id"))})})}function E(s=""){c(`https://your-energy.b.goit.study/api/exercises/${s}`,null,!1).then(e=>o(e)).catch(e=>console.log(e));function o(e){const a=`
<div data-state="open"
     class="fixed inset-0 z-50 bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0"
     style="pointer-events: auto;" data-aria-hidden="true" aria-hidden="true"></div>
...
`;document.getElementById("dialog").innerHTML=a,document.getElementById("close-dialog").addEventListener("click",()=>{document.getElementById("dialog").innerHTML=""}),document.getElementById("give-rating").addEventListener("click",()=>{L(s)})}}const g="https://your-energy.b.goit.study/api/exercises";class x{constructor(o=[]){}execute(o=""){c(o?g:g+o,null,!1).then(e=>v(e)).catch(e=>console.log(e))}}function B(s){const o=document.getElementById("renderer"),e=s.results.map(({name:a,imgURL:t})=>`
      ...
      `).join("");o.innerHTML=`<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5" id="muscles">${e}</div>`,f(s.totalPages,"Muscles"),document.querySelectorAll(".read-more").forEach(a=>{a.addEventListener("click",()=>{new x().execute(),console.log("?bodypart=back")})})}class u{constructor(o){this.api="https://your-energy.b.goit.study/api/filters?filter="+o}execute(o=1){c(this.api,o).then(e=>B(e)).catch(e=>console.log(e))}}const p={muscles:new u("Muscles"),exercises:new x,equipment:new u("Equipment"),body_parts:new u("Body parts")},j="https://your-energy.b.goit.study/api/exercises?keyword=";function M(s){c(j+s).then(o=>v(o)).catch(o=>console.log(o))}const z=document.querySelectorAll("#filters-strategy button"),A=document.querySelector("#search");A.addEventListener("input",s=>M(s.target.value));z.forEach(s=>{s.addEventListener("click",o=>{let e=o.target.getAttribute("data-filter");p[e.toLowerCase()].execute()})});p.muscles.execute();function h(s){const o=document.getElementById("quote-text"),e=document.getElementById("quote-author");o.insertAdjacentHTML("beforeend",s.quote),e.insertAdjacentHTML("beforeend",s.author)}const $="https://your-energy.b.goit.study/api/quote",w="savedQuote",m=localStorage.getItem(w);m?h(JSON.parse(m)):c($,null,!1).then(s=>{localStorage.setItem(w,JSON.stringify(s)),h(s)}).catch(s=>console.log(s));
//# sourceMappingURL=quote-SPqCr_8q.js.map
